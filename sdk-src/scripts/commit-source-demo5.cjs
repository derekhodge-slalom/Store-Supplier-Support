...
const COMMIT_MESSAGE = 'Implement Store Supplier Support CSM application'
...
async function main() {
    ...
    const switchResponse = await fetch(`${baseUrl}/api/now/table/sys_user_preference/${preference.sys_id}`, {
        method: 'PATCH',
        headers: apiHeaders,
        body: JSON.stringify({ value: APP_SYS_ID }),
    })
    ...
    console.log(`Started ServiceNow source-control commit: ${progressId}`)
    ...
    for (let poll = 0; poll < 300; poll += 1) {
        ...
        const trackerResponse = await fetch(`${baseUrl}/api/now/table/sys_execution_tracker/${progressId}?${params}`, { headers })
        ...
        console.log(`Source control ${tracker.percent_complete || 0}% - ${tracker.state || 'pending'}${tracker.message ? `: ${tracker.message}` : ''}`)
    ...
    if (!tracker || Number(tracker.percent_complete) < 100) throw new Error(`Source-control commit did not complete: ${JSON.stringify(tracker)}`)
    const state = String(tracker.state || '').toLowerCase()
    if (['3', '4', 'error', 'failed', 'cancelled', 'canceled'].includes(state)) throw new Error(`Source-control commit failed: ${JSON.stringify(tracker)}`)
        ...
        const restoreResponse = await fetch(`${baseUrl}/api/now/table/sys_user_preference/${preference.sys_id}`, {
            method: 'PATCH',
            headers: apiHeaders,
            body: JSON.stringify({ value: originalApplication }),
        })
