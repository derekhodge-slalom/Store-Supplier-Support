...
async function main() {
    ...
    await check('Slalom-branded Employee Center', 'sp_portal', 'url_suffix=esc^theme=e1c0a4c2d6b6494f9a887a8f171bc3d2^logo_alt_text=Slalom', 1)
    ...
    await check('Usable Store Supply model display names', 'x_sln_store_suppli_store_supply_model', 'demo_data=true^display_nameSTARTSWITHDemo ', 24)
    ...
    await check('Native Store Supply model categories', 'cmdb_model_category', 'nameSTARTSWITHStore Supply - ^asset_class=alm_consumable^product_model_class=x_sln_store_suppli_store_supply_model^allow_in_bundle=true', 6)
    ...
    await check('Categorized demo store supply inventory', 'alm_consumable', 'display_nameSTARTSWITHDEMO ^model.sys_class_name=x_sln_store_suppli_store_supply_model^model_category.nameSTARTSWITHStore Supply - ', 51)
